# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/methods/00_losses_metrics.ipynb.

# %% auto 0
__all__ = ['kl_divergence', 'beta_mse_loss', 'btcvae_loss', 'get_mig', 'betaLoss_C_scheduler', 'save_logvars', 'save_air_logvars',
           'save_bvae_logvars', 'save_separate_losses']

# %% ../nbs/methods/00_losses_metrics.ipynb 2
import torch
...
# %% ../nbs/methods/00_losses_metrics.ipynb 5
def kl_divergence(mu, 
                  logvar):
    ''' 
    Computes the Kullback-Leibler divergence between a Gaussian distribution N(mu, exp(logvar)) and the normal distribution N(0,1)
    '''
...
# %% ../nbs/methods/00_losses_metrics.ipynb 6
class beta_mse_loss(torch.nn.modules.loss._Loss):

    '''
    Beta-VAE loss function with Mean Squared Error as reconstruction loss.
    Implements both the original loss from Higgins et al. (2017) and the modified version from Burgess et al. (2017).   

    Parameters
    ----------
    loss_objective : str, optional
        'H' for original (Higgins et al.2017), 'B' for version (Burgess et al. 2017)
    beta : float, optional
        Regularizing factor in betaVAE
    gamma : float, optional
        Regularizing factor in Burgess betaVAE
    C_max : float, optional
        Maximum value of C
    C_stop_iter : int, optional
        When to reach the C_max
    recon_objective : str, optional
        either 'mse' (Mean Squared Error) or 'bce' (Binary Cross Entropy)
    reduction : str, optional
        reduction applied in the mse
    mse_as_konny : bool, optional
        if True, adapts the MSE loss to have exactly what they have in https:// github.com/1Konny/Beta-VAE/blob/master/solver.py 
    '''
...
# %% ../nbs/methods/00_losses_metrics.ipynb 11
class btcvae_loss(nn.modules.loss._Loss):
    """
    Beta-TC-VAE loss. Adapted from:
    Chen et al., "Isolating Sources of Disentanglement in Variational Autoencoders", 2018. (https://github.com/rtqichen/beta-tcvae)
    Adapted to have the same interface as `beta_mse_loss`.
    Main change is the use of the exact KL divergence rather than the sampled one for the dimension-wise KL term.
...
# %% ../nbs/methods/00_losses_metrics.ipynb 15
def get_mig(z, inp, true_dimensions, bins = 20, return_entropy = False, normalized_MI = False):
    
    '''
    Computes the Mutual Information Gap (MIG) metric as in:
    Chen et al., "Isolating Sources of Disentanglement in Variational Autoencoders", 2018. 

    Parameters
    ----------
    z : np.ndarray
        Latent representations (num_samples, z_dim)
    inp : torch.Tensor
        Ground truth factors (num_samples, true_dimensions)
    true_dimensions : int
        Number of ground truth factors
    bins : int, optional
        Number of bins to use for discretization
    return_entropy : bool, optional
        If True, also returns the entropy of each ground truth factor
    normalized_MI : bool, optional
        If True, uses the normalized mutual information score instead of the mutual information score
    Returns
    -------
    mi : np.ndarray
        Mutual information matrix (true_dimensions, z_dim)
    mig : float
        Mutual Information Gap
    entropy : list of float, optional
        Entropy of each ground truth factor (only if return_entropy is True)
    '''
...
# %% ../nbs/methods/00_losses_metrics.ipynb 18
...
# %% ../nbs/methods/00_losses_metrics.ipynb 20
...
# %% ../nbs/methods/00_losses_metrics.ipynb 21
...
# %% ../nbs/methods/00_losses_metrics.ipynb 22
...
# %% ../nbs/methods/00_losses_metrics.ipynb 24
